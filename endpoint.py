...
@app.route("/")
def home():
    """Render the main home page template, checking WorkOS session."""
    user_profile = None
    ...
    try:
        return render_template(
            ...
            display_name=display_name if is_authenticated else None # Pass display_name
        )
    except Exception as e:
...
@app.route("/login")
def login():
    """Redirect the user to WorkOS AuthKit to authenticate."""
    ...
    try:
        ...
        app.logger.info(f"Redirecting to WorkOS AuthKit for authentication.")
...
def auth_callback():
    """Handle the callback from WorkOS AuthKit after authentication."""
    if not workos_client or not WORKOS_COOKIE_PASSWORD:
...
@app.route("/logout")
def logout():
    """Log the user out using WorkOS AuthKit and clear the session cookie."""
    # Ensure WorkOS client and cookie password are available
    if not workos_client or not WORKOS_COOKIE_PASSWORD:
    ...
    try:
        ...
        app.logger.info(f"Redirecting user to WorkOS logout URL.")
...
def query():
    ...
    try:
        ...
        metadata = {
            ...
            "has_video_audio_context": bool(
                result_data.get("video_transcript_context")
            ),
            "has_audio_context": bool(result_data.get("audio_transcript_context")),
