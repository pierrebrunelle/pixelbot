...
def get_latest_news(topic: str) -> str:
    ...
    try:
        ...
        if response.status_code != 200:
            return f"Error: NewsAPI request failed with status {response.status_code}: {response.text}"
...
def search_news(keywords: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        print(f"DuckDuckGo search failed: {str(e)}")  # Log error
...
# Context Assembly UDF: Combines various text-based search results.
...
@pxt.udf
def _assemble_multimodal_context(
    question: str,
    tool_outputs: Optional[List[Dict[str, Any]]],
    doc_context: Optional[List[Union[Dict[str, Any], str]]],
    video_transcript_context: Optional[List[Union[Dict[str, Any], str]]],
    audio_transcript_context: Optional[List[Union[Dict[str, Any], str]]],
    ...
    (documents, video transcripts, audio transcripts, memory bank items,
    AND full chat history search results) relevant to the user's question.
    Does NOT include recent chat history (last N turns) or image/video frame details
    (which are passed separately to the final prompt assembly).
    """
    # --- Image Handling Note ---
    ...
    # Format video transcript context inline
    video_context_str = "N/A"
    if video_transcript_context:
        # Use a dictionary to track unique entries with highest similarity
        unique_entries = {}

        for item in video_transcript_context:
            if not isinstance(item, dict):
                continue

            text = item.get("text", "")
            source = item.get("source_video", "Unknown Video")
            sim_score = item.get("sim", 0)

            source_name = os.path.basename(str(source))

            # Create a unique key for this text
            unique_key = text.strip()

            # Only add this entry if it's new or has higher similarity than previous one
            if (
                unique_key not in unique_entries
                or sim_score > unique_entries[unique_key][1]
            ):
                unique_entries[unique_key] = (source_name, sim_score, text)

        # Convert the unique entries dictionary to formatted strings
        video_items = []
        for source_name, sim_score, text in unique_entries.values():
            if text:
                video_items.append(
                    f"- [Source: {source_name} | Sim: {sim_score:.3f}] {text}"
                )

        if video_items:
            video_context_str = "\n".join(video_items)

    # Format direct audio transcript context inline
    audio_context_str = "N/A"
    if audio_transcript_context:
        # Use a dictionary to track unique entries with highest similarity
        unique_audio_entries = {}

        for item in audio_transcript_context:
            if not isinstance(item, dict):
                continue

            text = item.get("text", "")
            source = item.get("source_audio", "Unknown Audio")
            sim_score = item.get("sim", 0)
            ...
            # Only add this entry if it's new or has higher similarity than previous one
            if (
                unique_key not in unique_audio_entries
                or sim_score > unique_audio_entries[unique_key][1]
            ):
                unique_audio_entries[unique_key] = (source_name, sim_score, text)

        # Convert the unique entries dictionary to formatted strings
        audio_items = []
        for source_name, sim_score, text in unique_audio_entries.values():
            if text:
                audio_items.append(
                    f"- [Source: {source_name} | Sim: {sim_score:.3f}] {text}"
                )
...
def assemble_final_messages(
    ...
    """
    Assembles the final list of messages for the LLM API, combining
    recent chat history (user/assistant turns) with the summarized text context,
    and correctly formats image and video frame data for multimodal input.
    """
    messages = []
